...
\texttt{transformador/primario/inrush}       & Flag + pico                 & A      \\
...
O inrush ocorre no instante da energização. A detecção é feita monitorando a corrente do primário com alta frequência de amostragem e verificando se ultrapassa um limiar configurável:

\begin{lstlisting}[style=cpp, caption={Detecção de Inrush Current}]
#define LIMIAR_INRUSH_A  5.0   // ajustar para a corrente nominal do trafo
#define JANELA_INRUSH_MS 500   // janela de tempo para captura (ms)

bool  inrushDetectado = false;
float picoInrush      = 0.0;

void verificarInrush(float correnteAtual) {
    static uint32_t tInicio = 0;
    static bool     monitorando = false;

    // Inicio do monitoramento ao detectar pico acima do limiar
    if (!monitorando && correnteAtual > LIMIAR_INRUSH_A) {
        monitorando      = true;
        tInicio          = millis();
        inrushDetectado  = true;
        picoInrush       = correnteAtual;
    }

    // Atualizar pico durante a janela
    if (monitorando) {
        if (correnteAtual > picoInrush) picoInrush = correnteAtual;
        if (millis() - tInicio > JANELA_INRUSH_MS) monitorando = false;
    }
}
\end{lstlisting}
...
Para $f_s = 500$~Hz e $N = 256$ amostras: $\Delta f = 500/256 = 1{,}95$~Hz, resolução suficiente para isolar 120~Hz dos harmônicos adjacentes. A biblioteca \texttt{arduinoFFT} implementa o algoritmo eficientemente no ESP32:

\begin{lstlisting}[style=cpp, caption={FFT com arduinoFFT}]
#include <arduinoFFT.h>

const uint16_t AMOSTRAS  = 256;
const float    FREQ_SAMP = 500.0;  // Hz

double vReal[AMOSTRAS], vImag[AMOSTRAS];
arduinoFFT FFT = arduinoFFT(vReal, vImag, AMOSTRAS, FREQ_SAMP);

float calcFFT_120hz() {
    // Preencher buffer com leituras do MPU6050 (eixo Z)
    for (uint16_t i = 0; i < AMOSTRAS; i++) {
        vReal[i] = lerAceleracaoZ();  // valor em g
        vImag[i] = 0;
        delayMicroseconds(1000000 / FREQ_SAMP);  // pausa MICRO, nao millis
    }
    ...
    // Indice correspondente a 120 Hz: (120 / 500) * 256 = 61.44 -> indice 61
    uint16_t idx120hz = (uint16_t)(120.0 / FREQ_SAMP * AMOSTRAS);
    return vReal[idx120hz];
}
\end{lstlisting}
...
\midrule
18/05 & 2ª Avaliação &
    Simulação no Wokwi ou Proteus com todos os componentes. Hardware virtual funcionando, captura de sinais, IHM local com dados simulados. Todos os requisitos demonstrados em software. \\
\midrule
...
\begin{itemize}[leftmargin=1cm, itemsep=2pt]
    \item \textbf{P1:} Circuito de condicionamento simulado com fonte AC virtual no Wokwi;
    \item \textbf{P2:} Firmware base compilando e rodando na simulação;
    \item \textbf{P3:} FFT calculando com dados mockados (seno 120~Hz gerado por software);
    \item \textbf{P4:} Broker Mosquitto ativo, ESP32 publicando dados no tópico correto;
